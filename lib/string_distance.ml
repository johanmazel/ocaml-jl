...
let utf8_code_array_of_string
    string
  =
  Array.of_list
    (List.map
       (fun char -> 
          (Batteries.UChar.code 
             (Batteries.UChar.of_char char)
          )
       )
       (Batteries.String.to_list string)
    )
...
let levenshtein_distance x y =
  ...
  distance_utf8 
    (utf8_code_array_of_string x)
    (utf8_code_array_of_string y)
